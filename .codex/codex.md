...
- Follow [project-change-plan](./skills/project-change-plan/SKILL.md) only when the user asks to read or check `project_changes.md` (including `project_change`). Do not start this workflow merely because the files exist or a general development or `change_planning.md` request is made.
